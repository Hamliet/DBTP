...
public class Enroll implements ActionListener {
	...
	private JLabel cs_birth = new JLabel("생일 (4자리 숫자)");
	private JLabel cs_phone = new JLabel("연락처 (4자리 숫자)");
	...
	private JLabel me_cost = new JLabel("가격 (최대 999999원)");
	...
	JPanel EnrollSearchPanel() {
		...
		cs_birth.setBounds(20, 70, 140, 30);
		cs_phone.setBounds(20, 130, 140, 30);

		cs_tf1.setBounds(190, 9, 80, 30);
		cs_tf2.setBounds(190, 70, 80, 30);
		cs_tf3.setBounds(190, 130, 80, 30);
		...
		me_cost.setBounds(20, 70, 140, 30);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == c_enroll) {
			...
			if (c_tf.getText().equals("") == true) {
				JOptionPane.showMessageDialog(null, (String) "검색할 이름을 입력하세요.",
						"Search Denided", 2);// 예외처리15
			} else if (c_tf.getText().equals("") == false) {
				...
				if (customer_found == false) {
					JOptionPane.showMessageDialog(null, (String) "검색 결과 없음.",
							"Customer Not Found", 2);// 예외처리2
				} else {

					sqlStr = "select customer_name, customer_id, customer_birth, customer_phone_number, customer_grade, customer_acc_cost from customer where customer_name = '"
							+ c_tf.getText() + "'";
					try {
			...
			boolean customer_found = false;
			if (cs_tf1.getText().equals("") == true
					|| cs_tf2.getText().equals("") == true
					|| cs_tf3.getText().equals("") == true) {
				JOptionPane.showMessageDialog(null, (String) "모든 값을 입력해 주세요.",
						"enroll denied", 2);// 예외처리19
			} else if (cs_tf1.getText().equals("") == false) {
				sqlStr = "select customer_name from customer";
				try {
					stmt = dbTest.prepareStatement(sqlStr);
					ResultSet rs2 = stmt.executeQuery(sqlStr);
				...
				if (customer_found == true) {
					JOptionPane.showMessageDialog(null,
							(String) "이미 존재하는 이름 입니다.", "enroll denied", 2); // 예외처리3
				} else {
					boolean check_integer = true;
					String name = cs_tf1.getText();
					String birth = cs_tf2.getText();
					String contact = cs_tf3.getText();
					Random random = new Random();
					try {
						if(Integer.parseInt(birth) >9999 || Integer.parseInt(contact)>9999){ //4자리 체크 예외처리20
							check_integer = false; 
						}
					...
					if (check_integer == false) {
						JOptionPane.showMessageDialog(null,
								(String) "올바른 숫자를 입력해 주세요.",
								"Enroll Denied", 2);// 예외처리12
					} else {
						try {
							stmt = dbTest.prepareStatement(sqlStr);
			...
			if (m_tf.getText().equals("") == true) {
				JOptionPane.showMessageDialog(null, (String) "검색할 메뉴를 입력하세요.",
						"Search Denied", 2);// 예외처리16
			} else if (m_tf.getText().equals("") == false) {
				...
				if (menu_found == false) {
					JOptionPane.showMessageDialog(null, (String) "검색 결과 없음.",
							"Menu Not Found", 2);// 예외처리4
				}

				else {

					sqlStr = "select menu_name, menu_price from menu where menu_name = '"
							+ m_tf.getText() + "'";
					try {
						stmt = dbTest.prepareStatement(sqlStr);
						rs = stmt.executeQuery();
			...
			if (me_tf.getText().equals("")
					|| me_tf2.getText().equals("") == true) {
				JOptionPane.showMessageDialog(null, (String) "모든 값을 입력해 주세요.",
						"enroll denied", 2);// 예외처리18
			} else if (me_tf.getText().equals("") == false) {
				sqlStr = "select menu_name from menu";
				try {
					stmt = dbTest.prepareStatement(sqlStr);
					ResultSet rs2 = stmt.executeQuery(sqlStr);
				...
				if (menu_found == true) {
					...
					int i_price = 0;
					try {
						if(Integer.parseInt(me_tf2.getText()) > 999999){ //6자리 체크 예외처리21 
							check_integer = false;
						}
						
						sqlStr = "insert into menu values (" + "'" + menu
								+ "', " + price + ")";
						i_price = Integer.parseInt(me_tf2.getText());
					...
					if (check_integer == false) {
						JOptionPane.showMessageDialog(null,
								(String) "올바른 범위의 숫자만 입력해주세요.", "Enroll Denied", 2);// 예외처리13
					} else {
						try {
							stmt = dbTest.prepareStatement(sqlStr);
							stmt.execute(sqlStr);
						...
						mn.menu_price[menu_count] = i_price;
						menu_count++;
			...
			if (e_tf.getText().equals("") == true) {
				JOptionPane.showMessageDialog(null, (String) "검색할 이름을 입력하세요.",
						"Search Denied", 2);// 예외처리14
			} else if (e_tf.getText().equals("") == false) {
				...
				if (employee_found == false) {
					JOptionPane.showMessageDialog(null, (String) "검색 결과 없음.",
							"Employee Not Found", 2);// 예외처리7
				}

				else {

					sqlStr = "select employee_name, employee_position, total from employee where employee_name = '"
							+ e_tf.getText() + "'";
					try {
						stmt = dbTest.prepareStatement(sqlStr);
						rs = stmt.executeQuery();
			...
			boolean employee_found = false;
			if ((ee_tf.getText().equals("") == true)) {
				JOptionPane.showMessageDialog(null, (String) "이름을 입력해주세요.",
						"Enroll denied", 2);// 예외처리17
			} else if (ee_tf.getText().equals("") == false) {
				sqlStr = "select employee_name from employee";
				try {
					stmt = dbTest.prepareStatement(sqlStr);
					ResultSet rs2 = stmt.executeQuery(sqlStr);
				...
				if (employee_found == true) {
					JOptionPane.showMessageDialog(null,
							(String) "이미 존재하는 이름 입니다.", "enroll denied", 2);// 예외처리8
				} else {
